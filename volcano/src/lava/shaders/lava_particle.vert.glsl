...
varying vec3 v2f_spherical_pos;
...
void main() {
  ...
  float noise = perlin_noise(seed + vec2(position.x + position.z  , position.y  ));
  float noise_b = perlin_fbm(seed + vec2(position.x + position.z  , position.y));
  float radius = 1.1 + 0.9 * noise + 0.1 * noise_b;

  vec3 spherical_pos = vec3(theta, phi, radius);
  v2f_spherical_pos = spherical_pos;

  // Go back to cartesian coordinates
  shad_pos = spherical_pos.z *  vec3(
     sin(spherical_pos.y) * cos(spherical_pos.x),
  ...
  vec4 position_v4 = vec4((particle_radius * shad_pos) + particle_position, 1);
